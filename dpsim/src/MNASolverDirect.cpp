...
template <typename VarType>
void MnaSolverDirect<VarType>::solveIteratively(Real time, Int timeStepCount) {
	...
	do{
		...
		Matrix leftStep = mDirectLinearSolverVariableSystemMatrix->solve(mRightSideVector);
		**mLeftSideVector += leftStep;
		...
		for(auto comp : mMNAComponents){
			comp->mnaCompUpdateVoltage(**mLeftSideVector);
			comp->mnaCompUpdateCurrent(**mLeftSideVector);
		}
	...
	// Components' states will be updated (again) by the post-step tasks
	// which is not necessary as it is done at the end of each iteration
}
...
template <typename VarType>
std::shared_ptr<CPS::Task> MnaSolverDirect<VarType>::createSolveTaskIterative()
{
	return std::make_shared<MnaSolverDirect<VarType>::SolveTaskIterative>(*this);
}
