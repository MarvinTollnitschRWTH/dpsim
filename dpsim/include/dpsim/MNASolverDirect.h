...
namespace DPsim {
	...
	protected:
		...
		std::shared_ptr<CPS::Task> createSolveTaskIterative() override;
	...
	public:
		...
		///
		class SolveTaskIterative : public CPS::Task {
		public:
			SolveTaskIterative(MnaSolverDirect<VarType>& solver) :
				Task(solver.mName + ".Solve"), mSolver(solver) {
			...
			void execute(Real time, Int timeStepCount) {
				mSolver.solveIteratively(time, timeStepCount);
				mSolver.log(time, timeStepCount);
			}
